...
//! ### `context`
//!
//! A top-level `context` changes the trait generated by `SchemaRead` from [`SchemaRead`] to
//! [`SchemaReadContext`]. Mark each field that should receive the context with
//! `#[wincode(context)]`; unmarked fields continue to use their ordinary [`SchemaRead`]
//! implementation. The field marker can also be combined with `with`, for example
//! `#[wincode(with = "MyAdapter", context)]`.
//!
//! Lifetimes appearing in the context type remain tied to the context instead of the serialized
//! input. This allows data derived from the context to outlive the input bytes while ordinary
//! borrowed fields remain tied to them.
//!
//! `SchemaWrite` is unchanged: field-level contexts only select how fields are read.
//! `UninitBuilder` does not currently support types with a top-level context.
//!
//! Example:
//! ```
//! # #[cfg(all(feature = "derive", feature = "bumpalo"))] {
//! use bumpalo::{Bump, collections::String};
//! use wincode::{SchemaRead, SchemaWrite, deserialize_with_context, serialize};
//!
...
//! let message = Message {
//!     id: 42,
//!     text: String::from_str_in("hello", &bump),
//! };
//!
//! let decoded: Message = {
...
//! |`context`|`bool`|`false`|Reads the field using [`SchemaReadContext`] and the top-level `context`. Requires a top-level `context` type.|
//!
//! ### `skip`
//!
