...
use {
    crate::{
        ...
        float_encoding::{FloatEncoding, ZeroCopyFloatEncoding},
...
macro_rules! impl_float {
    ($($ty:ty => $validate_write:ident, $validate_read:ident),* $(,)?) => {
        $(
            unsafe impl<C: ConfigCore> ZeroCopy<C> for $ty
            where
                C::ByteOrder: PlatformEndian,
                C::FloatEncoding: ZeroCopyFloatEncoding,
            {}
            ...
            unsafe impl<C: ConfigCore> SchemaWrite<C> for $ty {
                ...
                const TYPE_META: TypeMeta = TypeMeta::Static {
                    size: size_of::<$ty>(),
                    zero_copy: C::FloatEncoding::ZERO_COPY
                        && {
                            #[cfg(target_endian = "big")]
                            { matches!(C::ByteOrder::ENDIAN, Endian::Big) }
                            #[cfg(target_endian = "little")]
                            { matches!(C::ByteOrder::ENDIAN, Endian::Little) }
                        },
                };
                ...
                fn write(mut writer: impl Writer, src: &Self::Src) -> WriteResult<()> {
                    C::FloatEncoding::$validate_write(*src)?;
            ...
            unsafe impl<'de, C: ConfigCore> SchemaRead<'de, C> for $ty {
                ...
                fn read(mut reader: impl Reader<'de>, dst: &mut MaybeUninit<Self::Dst>) -> ReadResult<()> {
                    ...
                    C::FloatEncoding::$validate_read(val)?;
                    dst.write(val);
                    Ok(())
                }
            }
...
impl_float!(
    f32 => validate_f32_write, validate_f32_read,
    f64 => validate_f64_write, validate_f64_read,
);
