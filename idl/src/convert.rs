//! IDL spec conversion.
//!
//! The public [`convert_idl`] entry point normalizes current-spec IDLs and
//! retains legacy-input parsing for compatibility with existing
//! `declare_program!` consumers.
            ...
            let seed = match value {
                ...
                IdlSeed::Const(seed) => Self::Const(t::IdlSeedConst {
                    value: match seed.ty {
                        ...
                        // Legacy byte seeds are represented as JSON arrays of
                        // u8 values.
                        IdlType::Bytes => {
